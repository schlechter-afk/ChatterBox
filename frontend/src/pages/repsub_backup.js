  ...
  return (
    ...
    <AppBar position="static" className={stylecomp.appBar}>
      ...
      {/* <Navbar/> */}    
        ...
        <div className='bgrepeatlogsubreddit' style={{height:"100vh"}}>
          <div style={{display:"flex",flexWrap:"wrap"}}>
            {savearr.map((follower, index) => (
              <div key={index} style={mystyle2}>
                  ...
                  <button disabled={follower.action == "ignored"} onClick={()=>blockfunc(follower)}> Block User </button> <br/><br/>
                  <button disabled={follower.action == "ignored"} onClick={()=>ignorefunc(follower)}> {follower.action == "ignored" && "Ignored"} {follower.action == "temp" && "Ignore Report"}   </button> <br/><br/>
              </div>
            ))}
          </div>
        </div>
